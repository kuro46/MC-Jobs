...
public class PlayerCache implements Serializable {
	...
	public static boolean removeJob(String player, String job){
		...
		if(checkPlayer.playerJobs.contains(job)){
			...
			if (p != null) {
				p.sendMessage(McJobs.getPlugin().getLanguage().getExperience("reset").addVariables(job, player, ""));
            }
		...
		if(checkPlayer.jobexp.containsKey(job)){
			...
			if(Leveler.getXPtoLevel(level) <= value){
				while(Leveler.getXPtoLevel(level) <= value){
					value = value - Leveler.getXPtoLevel(level);
					...
					if(!checkPlayer.jobrank.get(job).equalsIgnoreCase(rank)){
                        ...
                        final Player p = Bukkit.getPlayer(player);
						if(p != null) {
							p.sendMessage(McJobs.getPlugin().getLanguage().getExperience("rank").addVariables(job, player, rank));
                        }
					}
                    ...
                    final Player p = Bukkit.getPlayer(player);
					if(p != null) {
						p.sendMessage(McJobs.getPlugin().getLanguage().getExperience("level").addVariables(job, player, level.toString()));
                    }
				}
	...
	public static boolean decrementTimer(String player){
		...
		while(it.hasNext()){
			...
			if(i < 1){
                ...
                final Player p = Bukkit.getPlayer(player);
				if(p != null){
					p.sendMessage(modText.getJobCommand("rejoin").addVariables(pair.getKey(), player, ""));
				}
